...
import joblib
GradientBoostingClassifier_model_filename = 'helper/GradientBoostingClassifier_model.joblib'
loaded_model_GradientBoostingClassifier = joblib.load(GradientBoostingClassifier_model_filename)
...
def predict_new_customer_churn_GradientBoostingClassifier(customer_data, threshold=0.5):
    import numpy as np
    # Convert input to numpy array
    data_array = np.array(customer_data, dtype=np.float32).reshape(1, -1)

    # Get churn probability (probability of class 1)
    probability = loaded_model_GradientBoostingClassifier.predict_proba(data_array)[:, 1][0]
